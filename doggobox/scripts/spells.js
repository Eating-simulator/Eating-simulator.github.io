var spells = {
    ...
    onload: function() {
        // We add the spells
        this.addSpell("Candies, faster doggi !", this.getFasterCandiesPrice.bind(this), this.fasterCandies.bind(this), "Congratulations ! You will now gain more doggi each second !", []);
        this.addSpell("Candies, more doggi !", this.getMoreCandiesPrice.bind(this), this.moreCandies.bind(this), "///", []);
    ...
    moreCandies: function() {
        var nbr = Math.floor(doggi.nbrTotal / 20);

        // We can't gain more doggi than the price we pay for the spell !
        if (nbr > 300000) nbr = 300000;

        doggi.setNbrOwned(doggi.nbrOwned + nbr);
        hut.setSpeech("Here's " + nbr + " doggi for you !");
    },
    ...
    setFasterCandiesFibo: function(prev, curr) {
        ...
        // Sort the spells list depending on their price, since the price of the faster doggi spell just changed
        this.sortListDependingOnPrice();
    },

    fasterCandies: function() {
        // We change doggi per second
        doggi.setCandiesPerSecond(this.fasterCandiesFiboCurr);
