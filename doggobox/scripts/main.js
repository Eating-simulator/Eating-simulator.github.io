var main = {
    ...
    oneTenthSecInterval: function() {
        // We try to convert doggi into doges
        doggiConverter.convert();
    },

    secInterval: function() {
        // Candies
        if (objects.list.oldAmulet.have == false) doggi.setNbrOwned(doggi.nbrOwned + doggi.doggiPerSecond);
        else doggi.setNbrOwned(doggi.nbrOwned + doggi.doggiPerSecond * 3);
        ...
        if (farm.productionDelayType == "sec") {
            if (objects.list.hornOfPlenty.have == false) doges.setNbrOwned(doges.nbrOwned + farm.dogesProduction);
            else doges.setNbrOwned(doges.nbrOwned + farm.dogesProduction * 3);
        }
