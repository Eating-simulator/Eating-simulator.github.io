var potions = {
    ...
    onload: function() {
        ...
        this.addPotion("seed", "Seed", "#3dab3a", "potions.seed();", "This seed is able to make grow a doggo tree. The doggo tree is made of doggi, and it takes a lot of time to cut it down. Using the seed will grow a tree in front of you, if there's enough place.", "special");
    ...
    buyPotion: function(potion, price) {
        ...
        doggi.setNbrOwned(doggi.nbrOwned - price);
    ...
    makeClone: function(hp, max_hp) {
        return land.createAlly("\^w^", max_hp, hp, "cloned sword", "A clone of you.", []);
    },

    makeCandyTree: function() {
        var hp = 0;

        // One chance out of 100 to spawn the Yggdrasil ^w^
        if (random.oneChanceOutOf(100)) {
            return land.createTrap("/Y\\", 10000, 10000, "none", "Yggdrasill is its name, a tall tree, showered with shining loam.", [drops.createDrop("doggi", 10000)]);
        } else {
            hp = 500 + 100 * random.getRandomIntUpTo(4); // 500 / 600 / 700 / 800 / 900
            return land.createTrap("\\|/", hp, hp, "none", "A doggo tree. I hope you're carrying a good axe.", []);
        }
    },
