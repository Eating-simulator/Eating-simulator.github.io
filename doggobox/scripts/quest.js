var quest = {
    ...
    doggiFound: 0, // Number of doggi found during a quest
    ...
    begin: function(getLandFromList, landIndex) {
        ...
        if (land.ponyTime == false)
            this.things[0] = {
                type: "character",
                text: "\^w^",
                max_hp: this.getCharacterMaxHp(),
                hp: this.getCharacterMaxHp(),
                weapon: (sword.name),
                description: "You"
            };
        else
    ...
    getCharacterMaxHp: function() {
        return 100 + Math.floor(Math.pow(doggi.nbrEaten, 0.4) * 2.1); // This function means ~ one day of doggi eaten at 1cnd/s = +200 hp
    },
    ...
    checkIfDead: function(index, indexKiller) {
        ...
        if (this.things[index].hp <= 0) {
            ...
            // Else, we're a mob
            else if (this.things[index].type == "mob" && (this.things[index].text != "\^w^" || yourself.canSurpass == true)) {
    ...
    stopBerserk: function() {
        this.berserk = false;
        this.speed = this.getSpeed();
        this.things[this.getCharacterIndex()].text = "\^w^";
    },
    ...
    makeBasicChest: function() {
        return land.createMob("CHS", 80, 80, "none", "A chest !! Very rare.", [drops.createDrop("doggi", 300 + random.getRandomIntUpTo(500)), drops.createDrop("object", "key", true), drops.createDrop("object", "boots", random.oneChanceOutOf(3)), drops.createDrop("object", "swampMap", random.oneChanceOutOf(3)), drops.createDrop("object", "hutMap", random.oneChanceOutOf(3))]);
    },

    makeOpenChest: function() {
        return land.createMob("CHS", 1, 1, "none", "An open chest, full of doggi !", [drops.createDrop("doggi", 6000 + random.getRandomIntUpTo(2000))]);
    },
    ...
    makeCandyMonster: function() {
        ...
        return land.createAlly("CND", hp, hp, "exploding doggi", "A doggo monster. He throws doggi on his ennemies.", []);
    },
