var hut = {
    ...
    throwLollipops: function() {
        if (doges.nbrOwned >= 10) {
            doges.setNbrOwned(doges.nbrOwned - 10);
            this.setStep(1);
            this.setSpeech("I see that you have doges... I could cast spells for you, in exchange of some sweets... I looove doges!");
            this.updateOnPage();
        }
    },
    ...
    acceptProposition: function() {
        this.setStep(2);
        this.setSpeech("See by yourself. My prices are high, but the spells are great! (1 klp means 1000 doges)");
        this.updateOnPage();
    },
...
"
                ...
                text += "\n\n<button id=\"hut_throw_doges\" onClick=\"hut.throwLollipops();\">Throw 10 doges inside the hut</button>";
                break;
            default:
                text += "\
/\\)\\))/\\(/|)\\))/((||\\/)\\)\\\n\
...
"
    ...
    canThisSpellBeUsed: function(id) {
        ...
        if (spells.list[id].price() > doges.nbrOwned) return false;
    ...
    useSpell: function(id) {
        if (doges.nbrOwned >= spells.list[id].price()) {
            doges.setNbrOwned(doges.nbrOwned - spells.list[id].price());
