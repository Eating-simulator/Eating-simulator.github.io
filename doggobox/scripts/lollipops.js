var doges = {
    ...
    // Functions
    buy1: function() {
        if (doggi.nbrOwned >= shop.oneLollipopPrice && this.nbrInStock >= 1) {
            doggi.setNbrOwned(doggi.nbrOwned - shop.oneLollipopPrice);
            this.setNbrOwned(this.nbrOwned + 1);
            this.setNbrBought(this.nbrBought + 1);
            this.setNbrInStock(this.nbrInStock - 1);
            shop.setMerchantSpeech("Thanks for buyin'! Here's your " + this.getFlavour() + " flavor doge.");
        }
    },

    buy10: function() {
        if (doggi.nbrOwned >= shop.tenLollipopsPrice && this.nbrInStock >= 10) {
            doggi.setNbrOwned(doggi.nbrOwned - shop.tenLollipopsPrice);
            this.setNbrOwned(this.nbrOwned + 10);
            this.setNbrBought(this.nbrBought + 10);
            this.setNbrInStock(this.nbrInStock - 10);
            shop.setMerchantSpeech("Thanks for buyin'! Here's your ten doges. Various flavours.");
        } else shop.setMerchantSpeech("I'm sorry, we don't have enough doges in stock to sell you ten of them. We currently have " + this.nbrInStock + " doges in stock.");
    },
    ...
    getFlavour: function() {
        ...
        var unrealistic = ["leprechaun", "korrigan", "doge", "snow", "storm", "door", "dracula"];
    ...
    setNbrOwned: function(value) {
        this.nbrOwned = value;
        if (this.nbrOwned != 1) htmlInteraction.setInnerHtml("doges", "You have " + this.nbrOwned + " doges!");
        else htmlInteraction.setInnerHtml("doges", "You have 1 doge!");
        htmlInteraction.setElementVisibility("doges", true);
