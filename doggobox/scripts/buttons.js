var buttons = {
    ...
    checkQuestBuyingButtons: function() {
        ...
        if (shop.currentSwordButtonId != "none") { // If the merchant is selling a sword right now
            if (doggi.nbrOwned >= shop.currentSwordPrice) this.enableButton(shop.currentSwordButtonId);
            else htmlInteraction.disableButton(shop.currentSwordButtonId);
        }

        // Potions & scrolls
        if (doggi.nbrOwned >= 600) this.enableButton("buy_health_potion");
        else htmlInteraction.disableButton("buy_health_potion");

        if (doggi.nbrOwned >= 150) this.enableButton("buy_escape_potion");
        else htmlInteraction.disableButton("buy_escape_potion");

        if (doggi.nbrOwned >= 400) this.enableButton("buy_scroll");
        else htmlInteraction.disableButton("buy_scroll");
    },

    checkEncrustSwordButton: function() {
        // Show the encrust the depleted uranium sword button
        if (doggi.nbrOwned >= 101 && sword.name == "depleted uranium sword") {
    ...
    checkPolishSwordButton: function() {
        // Show the polish the depleted uranium sword button
        if (doges.nbrOwned >= 30 && sword.name == "doggo depleted uranium sword") {
    ...
    checkComputer: function() {
        ...
        if (doges.nbrOwned >= 100000000000) htmlInteraction.showButton("computer_bug_6");
        else htmlInteraction.hideButton("computer_bug_6");

        if (doges.nbrOwned >= 1000000000000) htmlInteraction.showButton("computer_bug_7");
        else htmlInteraction.hideButton("computer_bug_7");

        if (doges.nbrOwned >= 10000000000000) htmlInteraction.showButton("computer_bug_8");
        else htmlInteraction.hideButton("computer_bug_8");
    },

    checkCoatSwordButton: function() {
        // Show the coat the solidified rubber blbl sword button
        if (chocolateBars.nbrOwned >= 1 && sword.name == "polished doggo depleted uranium sword") {
    ...
    checkLollipopsStockShortage: function() {
        ...
        if (shop.shown) {
            // If there's a doges stock shortage
            if (doges.stockShortage == true) {
                // We show the stock shortage and hide the buttons used to buy doges
                htmlInteraction.setElementVisibility("doges_stock_shortage", true);
                htmlInteraction.hideButton("buy_1_doge");
                htmlInteraction.hideButton("buy_10_doges");
            }
            // Else, no doges stock shortage
            else {
                // We don't show the stock shortage
                htmlInteraction.setElementVisibility("doges_stock_shortage", false);
                // We show and maybe enable the button to buy one doge
                htmlInteraction.showButton("buy_1_doge");
                if (doggi.nbrOwned >= shop.oneLollipopPrice) this.enableButton("buy_1_doge");
                else htmlInteraction.disableButton("buy_1_doge");
                // We maybe show and maybe enable the button to buy 500 doges
                if (doggi.nbrOwned >= shop.tenLollipopsPrice || shop.buy10LollipopsButtonShown) {
                    shop.setBuy10LollipopsButtonShown(true);
                    if (htmlInteraction.isElementVisible("buy_10_doges") == false) { // If it wasn't shown yet
                        htmlInteraction.showButton("buy_10_doges");
                        shop.setMerchantSpeech("There's now a discount for 10 doges! Buy them please.. I need candiiiies!");
                    }
                    this.enableButton("buy_10_doges");
                }
                if (doggi.nbrOwned < shop.tenLollipopsPrice) htmlInteraction.disableButton("buy_10_doges");
            }
        }
    },
    ...
    checkObjects: function() {
        ...
        // Check the doggi converter visibility
        doggiConverter.checkVisibility();
    ...
    checkLollipopsPlantingButtons: function() {
        // If we have the key of the doge farm
        if (objects.list.key.have) {
            // innerHtml of the lp_buttons span

            // Plant 1000 button
            if (doges.nbrOwned >= 1000 && farm.plantingButtonsStep < 4) {
                farm.setPlantingButtonsStep(4);
            }
            // Plant 100 button
            else if (doges.nbrOwned >= 100 && farm.plantingButtonsStep < 3) {
                farm.setPlantingButtonsStep(3);
            }
            // Plant 10 button
            else if (doges.nbrOwned >= 10 && farm.plantingButtonsStep < 2) {
    ...
    checkHut: function() {
        if (hut.shown) {
            switch (hut.step) {
                case 0:
                    htmlInteraction.showButton("hut_throw_doges");
                    if (doges.nbrOwned >= 10) {
                        this.enableButton("hut_throw_doges");
                    } else htmlInteraction.disableButton("hut_throw_doges");
                    break;
                case 2:
                    ...
                    if (quest.maxLandOrder == 7 && yourself.canSurpass == false) {
                        if (doges.nbrOwned >= 1000000) this.enableButton("hut_surpass");
    ...
    checkWishingWell: function() {
        if (wishingWell.shown) {
            switch (wishingWell.step) {
                case 0:
                    if (doggi.nbrOwned >= 1) {
                        this.enableButton("wishingWell_throw_doggo");
                    } else htmlInteraction.disableButton("wishingWell_throw_doggo");
                    break;
            }
        }
    },
