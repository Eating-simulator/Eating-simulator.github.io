var cauldron = {
    ...
    doggiInTheCauldron: 0, // Number of doggi present in the cauldron
    dogesInTheCauldron: 0, // Idem for doges
    weAreMixing: false, // True if the player is mixing right now
    weAreBoiling: false, // True if the player is boiling right now
    doggiWhenWeBeganAction: 0, // Number of doggi in the cauldron when we began mixing
    dogesWhenWeBeganAction: 0, // Idem doges
    actionsList: [{
        type: "none"
    }, {
        type: "none"
    ...
    registerAction: function(type, nbrCandies, nbrLollipops, timer) {
        ...
        this.actionsList.push({
            type: type,
            nbrCandies: nbrCandies,
            nbrLollipops: nbrLollipops,
            timer: timer
        });
    ...
    drawActionsInfo: function() {
        ...
        // Add doggi and doges info
        this.textActionsInfo += "    Candies :   " + doggi.nbrOwned;
        this.textActionsInfo += "<br/>    Lollipops : " + doges.nbrOwned;
    },

    drawActionsPut: function() {
        this.textActionsPut = "<b>What you want to put in the cauldron :</b><br/><br/>"
        this.textActionsPut += "<input id=\"cauldron_doggi_quantity\" type=\"text\" size=\"10\"/> doggi<br/>";
        this.textActionsPut += "<input id=\"cauldron_doges_quantity\" type=\"text\" size=\"10\"/> doges<br/>";
    ...
    drawActionsInCauldron: function() {
        ...
        this.textActionsInCauldron += "    Candies :   " + this.doggiInTheCauldron;
        this.textActionsInCauldron += "<br/>    Lollipops : " + this.dogesInTheCauldron + "<br/>";
    },
    ...
    setActionTimer: function(value) {
        ...
        if (this.weAreMixing) {
            if (this.actionTimer < 60) {
                ...
                if (this.actionTimer < 5 || (this.doggiInTheCauldron != 0 || this.dogesInTheCauldron != 0)) {
                    // We show the timer
    ...
    setWeAreMixing: function(value) {
        ...
        if (value == false && this.weAreMixing == true) {
            ...
            this.registerAction("mix", this.doggiWhenWeBeganAction, this.dogesWhenWeBeganAction, this.actionTimer);
        }
        // Else, if we want to begin mixing
            ...
            // We store some info
            this.doggiWhenWeBeganAction = this.doggiInTheCauldron;
            this.dogesWhenWeBeganAction = this.dogesInTheCauldron;
        }
    },
    ...
    setWeAreBoiling: function(value) {
        ...
        if (value == false && this.weAreBoiling == true) {
            ...
            this.registerAction("boil", this.doggiWhenWeBeganAction, this.dogesWhenWeBeganAction, this.actionTimer);
        }
    ...
    putIntoBottles: function() {
        ...
        if (lastAc.type == "mix" // Last action was mixing
            &&
            lastAc.nbrLollipops == 0 // We mixed no doge
            &&
            lastAc.nbrCandies > 0 // We mixed at least one doggo
            &&
            lastAc.nbrCandies % 100 == 0 // The doggi we mixed were a multiple of 100
            &&
            lastAc.nbrCandies == this.doggiInTheCauldron && lastAc.nbrLollipops == this.dogesInTheCauldron // We didn't add anything while mixing
            &&
            ...
            resultsList.push({
                type: "minor health",
                nbr: Math.floor(lastAc.nbrCandies / 100)
        ...
        if (lastAc.type == "mix" // Last action was mixing
            &&
            lastAc.nbrLollipops > 0 // We mixed at least one doge
            &&
            lastAc.nbrLollipops % 100 == 0 // The doges we mixed were a multiple of 100
            &&
            lastAc.nbrCandies == 0 // We mixed no doggo
            &&
            lastAc.nbrLollipops == this.dogesInTheCauldron // We didn't add any doge while mixing
            &&
            this.doggiInTheCauldron == lastAc.nbrLollipops // While mixing, we added as many doggi as we had doges at the beginning
            &&
            ...
            resultsList.push({
                type: "major health",
                nbr: Math.floor(lastAc.nbrLollipops / 100)
            }); // We add the result to the list
        }
        ...
        if (
            ...
            &&
            lastAc.nbrCandies == 0 // We boiled no doggo
            &&
            lastAc.nbrLollipops > 0 // We boiled at least one doge
            &&
            lastAc.nbrLollipops % 20000 == 0 // The doges we boiled were a multiple of 20000
            &&
            lastAc.nbrLollipops == this.dogesInTheCauldron // We didn't add any doge while boiling
            &&
            lastAc.nbrCandies == this.doggiInTheCauldron // We didn't add any doggo while boiling
            &&
            ...
            &&
            lastLastLastAc.nbrCandies == 0 // We boiled no doggo
            &&
            lastLastLastAc.nbrLollipops > 0 // We boiled at least one doge
            &&
            lastLastLastAc.nbrLollipops % 10000 == 0 // The doges we boiled were a multiple of 10000
            &&
            lastLastLastAc.timer >= 15 && lastLastLastAc.timer < 32 // It was boiling when we stopped boiling
            /* STUFF BETWEEN ACTIONS */
            &&
            lastAc.nbrLollipops == 2 * lastLastLastAc.nbrLollipops) { // We boiled at the end twice more doges than what we boiled at first
            potions.getPotions(potions.list.turtle, Math.floor(lastAc.nbrLollipops / 20000)); // We add the potions to our stock
            resultsList.push({
                type: "turtle",
                nbr: Math.floor(lastAc.nbrLollipops / 20000)
            }); // We add the result to the list
        }
        ...
        if (lastAc.type == "mix" // Last action was mixing
            ...
            lastAc.nbrCandies > 0 // We mixed at least one doggo
            &&
            lastAc.nbrCandies % 2000 == 0 // The doggi we mixed were a multiple of 2000
            &&
            lastAc.nbrCandies == this.doggiInTheCauldron && lastAc.nbrLollipops == this.dogesInTheCauldron // We didn't add anything while mixing
            &&
            ...
            resultsList.push({
                ...
                nbr: Math.floor(lastAc.nbrCandies / 2000)
            }); // We add the result to the list
        }
        ...
        if (lastAc.type == "boil" // Last action was boiling
            &&
            lastAc.nbrLollipops == 0 // We boiled no doge
            &&
            lastAc.nbrCandies == 0 // We boiled no doggo
            &&
            this.dogesInTheCauldron == 0 // We didn't add any doge while boiling
            &&
            this.convertCandiesToPotionsForTheCloningPotion(this.doggiInTheCauldron) > 0 // With the doggi we added while boiling (or after), we can make at least one potion
            &&
            lastAc.timer >= 32) { // The water burnt while boiling
            potions.getPotions(potions.list.cloning, this.convertCandiesToPotionsForTheCloningPotion(this.doggiInTheCauldron)); // We add the potions to our stock
            resultsList.push({
                type: "cloning",
                nbr: this.convertCandiesToPotionsForTheCloningPotion(this.doggiInTheCauldron)
            }); // We add the result to the list
        }
        ...
        if (lastAc.type == "mix" // Last action was mixing
            ...
            lastAc.nbrLollipops > 0 // We mixed at least one doge
            &&
            lastAc.nbrLollipops % 500 == 0 // The doges we mixed were a multiple of 500
            &&
            lastAc.nbrCandies == 10000 // We mixed 10000 doggi
            &&
            this.doggiInTheCauldron == lastAc.nbrCandies // We didn't add any doggo while mixing
            &&
            this.dogesInTheCauldron == lastAc.nbrLollipops) { // We didn't add any doge while mixing
            potions.getPotions(potions.list.gmooh, Math.floor(lastAc.nbrLollipops / 500)); // We add the potions to our stock
            resultsList.push({
                type: "G.M.O.O.H.",
                nbr: Math.floor(lastAc.nbrLollipops / 500)
            }); // We add the result to the list
        }
        ...
        if (lastAc.type == "boil" // Last action was boiling
            ...
            this.dogesInTheCauldron == 0 // We didn't add any doge while boiling
            &&
            this.doggiInTheCauldron > 0 // We added at least one doggo while or after boiling
            &&
            this.doggiInTheCauldron % 650 == 0) { // The doggi we added while or after boiling are a multiple of 650
            potions.getPotions(potions.list.seed, Math.floor(this.doggiInTheCauldron / 650)); // We add the potions to our stock
            resultsList.push({
                type: "seed",
                nbr: Math.floor(this.doggiInTheCauldron / 650),
                special: true,
                plural: "seeds"
            }); // We add the result to the list
        }
        ...
        if (
            ...
            /* LAST LAST LAST ACTION */
            &&
            lastLastLastAc.nbrLollipops == 0 // No doge
            &&
            lastLastLastAc.nbrCandies > 0 // At least one doggo
            &&
            lastLastLastAc.nbrCandies % 600 == 0 // Candies multiple of 600
            /* LAST LAST ACTION */
            &&
            lastLastAc.nbrLollipops > 0 // At least one doge
            &&
            lastLastAc.nbrLollipops % 6000 == 0 // Lollipops mutiple of 6000
            &&
            lastLastAc.nbrLollipops == 10 * lastLastLastAc.nbrCandies // Lollipops = 10*doggi of last last last action
            &&
            lastLastAc.nbrCandies == lastLastLastAc.nbrCandies // As many doggi as last last last action
            /* LAST ACTION */
            &&
            lastAc.nbrLollipops == lastLastAc.nbrLollipops // As many doges as last last action
            &&
            lastAc.nbrCandies == lastLastLastAc.nbrCandies * 2 // Twice more doggi than last last last action
            &&
            lastAc.nbrLollipops == this.dogesInTheCauldron // We didn't add any doge
            &&
            lastAc.nbrCandies == this.doggiInTheCauldron) { // We didn't add any doggo
            potions.getPotions(potions.list.jelly, Math.floor(lastAc.nbrCandies / 600)); // We add the potions to our stock
            resultsList.push({
                type: "jelly",
                nbr: Math.floor(lastAc.nbrCandies / 600),
                special: true,
                plural: "jellies"
            }); // We add the result to the list
        }
    ...
    putInTheCauldron: function() {
        ...
        var doggiInput = htmlInteraction.getElement("cauldron_doggi_quantity").value;
        var dogesInput = htmlInteraction.getElement("cauldron_doges_quantity").value;

        // We get the quantities
        if (doggiInput != "") {
            var doggiQuantity = parseInt(doggiInput);
        } else doggiQuantity = 0;

        if (dogesInput != "") {
            var dogesQuantity = parseInt(dogesInput);
        } else dogesQuantity = 0;

        // If the quantities are incorrect
        if (isNaN(doggiQuantity) || isNaN(dogesQuantity)) {
            // If both values are incorrect
            if (isNaN(doggiQuantity) && isNaN(dogesQuantity)) {
                htmlInteraction.setInnerHtml("cauldron_comment", "The values you entered are not numbers.");
            }
            // If only the doggi value is incorrect
            else if (isNaN(doggiQuantity)) {
                htmlInteraction.setInnerHtml("cauldron_comment", "The value you entered for doggi is not a number.");
            }
            // Else, only the doges value is incorrect
            else {
                htmlInteraction.setInnerHtml("cauldron_comment", "The value you entered for doges is not a number.");
            }
        }
        // Else, if we don't have enough doggi or doges to put all that in the cauldron
        else if (doggiQuantity > doggi.nbrOwned || dogesQuantity > doges.nbrOwned) {
        ...
        else if (doggiQuantity < 0 || dogesQuantity < 0) {
        ...
        // Else, we put all that in the cauldron !
            ...
            htmlInteraction.getElement("cauldron_doggi_quantity").value = "";
            htmlInteraction.getElement("cauldron_doges_quantity").value = "";
            // We substract doggi from our stock
            doggi.setNbrOwned(doggi.nbrOwned - doggiQuantity);
            doges.setNbrOwned(doges.nbrOwned - dogesQuantity);
            // And we add them in the cauldron
            this.setCandiesInTheCauldron(this.doggiInTheCauldron + doggiQuantity);
            this.setLollipopsInTheCauldron(this.dogesInTheCauldron + dogesQuantity);
        }
    },

    setCandiesInTheCauldron: function(value) {
        this.doggiInTheCauldron = value;
        this.updateActionsInCauldronOnPage();
    },

    setLollipopsInTheCauldron: function(value) {
        this.dogesInTheCauldron = value;
        this.updateActionsInCauldronOnPage();
    },
