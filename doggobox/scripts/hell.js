var hell = {
        ...
        for (var i = 0; i < 7; i++) {
            if (i != this.bufferPosition) {
                ...
                for (var j = this.size - 2; j >= 0; j--) {
                    if (this.buffers[i][j].text == "\^w^" && this.buffers[i][j].type == "ally" && this.buffers[i][j + 1].type == "none" && this.buffers[i][j].text != " * " && this.buffers[i][j].text != "|-|") {
                        this.buffers[i][j + 1] = this.buffers[i][j];
                        this.buffers[i][j] = quest.makeNoneThing();
                    }
                }
    ...
    makeDevil: function() {
        return land.createMob("DEV", 250, 250, "religion", "It's the devil itself !", [drops.createDrop("doggi", 100000)]);
    },
