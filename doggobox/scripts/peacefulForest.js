var peacefulForest = {
    ...
    move: function() {
        ...
        for (var i = 0; i < quest.things.length; i++) {
            // If it's a wood poney (wood poneeeey ^w^)
    ...
    load: function() {
        ...
        for (var i = 1; i < quest.things.length; i++) {
            if (random.flipACoin()) {
                ...
                else quest.things[i] = land.createMob("%%%", 5, 5, "none", "A tree. It sometimes drops a doggo.", [drops.createDrop("doggi", random.getRandomIntUpTo(1)), drops.createDrop("object", "key", random.oneChanceOutOf(2))]);
            }
        }
    ...
    makeWoodPoney: function() {
        return land.createMob("WPY", 12, 12, "hooves", "A wood poney ! It's a poney ! It the woods !", [drops.createDrop("doggi", 42)]);
    }
