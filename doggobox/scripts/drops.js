var drops = {
    ...
    getText: function() {
        ...
        // Candies found
        if (quest.doggiFound != 1) text += "You found " + quest.doggiFound + " doggi.";
        else text += "You found 1 doggo.";
    ...
    gainDrops: function() {
        // Gain the doggi
        doggi.setNbrOwned(doggi.nbrOwned + quest.doggiFound);
    ...
    getAllDropsFromList: function(list) {
        for (var i = 0; i < list.length; i++) {
            switch (list[i].type) {
                case "doggi":
                    quest.setCandiesFound(quest.doggiFound + list[i].param1);
                    break;
                case "object":
